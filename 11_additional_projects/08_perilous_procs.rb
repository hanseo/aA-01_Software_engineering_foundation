...
def multi_map(arr, n=1, &prc)
  new_arr = arr
  n.times { new_arr = new_arr.map(&prc) }
  new_arr
end
...
def proctition(arr, &prc)
  arr.select { |el| prc.call(el) } + filter_out(arr, &prc)
end
...
def chain_map(n, arr)
  arr.inject(n) { |acc, prc| prc.call(acc) }
end
...
def procipher(str, hash)
  ...
  str.split.each do |w|
    new_w = w
    hash.each do |k, v|
      new_w = v.call(new_w) if k.call(w)
    end
